...
freeze_data <- read.csv("scrap.csv")
...
injury_percent <- mutate (leakage_percent, injury = ((leakage-leakage_percent[1,12])/(100-leakage_percent[1,12])*100))
...
adjusted_injury_percent <- mutate (injury_percent, adjusted_injury = (injury/injury_percent[6,13])*100)
